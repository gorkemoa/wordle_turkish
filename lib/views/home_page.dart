          ...
          builder: (context, setState) {
            return AlertDialog(
              ...
              if (userStats != null) ...[
                ...
                const SizedBox(height: 16),
                _buildProfileInfo('Kazanma Serisi', userStats!['currentStreak'].toString()),
                const SizedBox(height: 16),
                _buildProfileInfo('En İyi Seri', userStats!['bestStreak'].toString()),
              ],
            ],
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (newName.length < 2) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Kullanıcı adı en az 2 karakter olmalıdır!'),
                ...
                final success = await FirebaseService.updateUserDisplayName(uid, newName);
                if (success && mounted) {
                  setState(() {});
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
